...
@njit(parallel=True)
def generate_dense_keys_zipf_sok(num_samples, num_slots, vocabulary_range_per_slot, key_dtype, alpha):
    dense_keys = np.empty((num_samples, num_slots, 1), key_dtype)
    for i in range(num_slots):
        vocab_range = vocabulary_range_per_slot[i]
        H = vocab_range[1] - vocab_range[0] + 1
        L = 1
...
@njit(parallel=True)
def generate_dense_keys_uniform_sok(num_samples, num_slots, vocabulary_range_per_slot, key_dtype):
    dense_keys = np.empty((num_samples, num_slots, 1), key_dtype)
    for i in range(num_slots):
        vocab_range = vocabulary_range_per_slot[i]
        keys_per_slot = np.random.randint(low=vocab_range[0], high=vocab_range[1], size=(num_samples)).astype(key_dtype)
...
def generate_random_samples_sok(num_samples, vocabulary_range_per_slot, dense_dim, key_dtype, alpha):
    vocabulary_range_per_slot = np.array(vocabulary_range_per_slot)
    num_slots = vocabulary_range_per_slot.shape[0]
    if alpha == 0:
        cat_keys = generate_dense_keys_uniform_sok(num_samples, num_slots, vocabulary_range_per_slot, key_dtype)
    else:
        cat_keys = generate_dense_keys_zipf_sok(num_samples, num_slots, vocabulary_range_per_slot, key_dtype, alpha)
    dense_features, labels = generate_cont_feats(num_samples, dense_dim)
    return cat_keys, dense_features, labels
