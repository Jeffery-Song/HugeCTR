...
selected_col += ['gpu_num']
selected_col += ['cache_intersect_ratio']
selected_col += ['cache_hit_ratio', 'cache_hit_overlap_ratio', 'cache_miss_overlap_ratio']
...
if __name__ == '__main__':
  ...
  for inst in bench_list:
    inst : BenchInstance
    try:
      inst.vals['cache_intersect_ratio'] = handle_nan(inst.get_val('Step(average) L4 cache intersect ratio'))
      inst.vals['cache_hit_ratio'] = handle_nan(inst.get_val('Step(average) L4 cache hit ratio'))
      inst.vals['cache_hit_overlap_ratio'] = handle_nan(inst.get_val('Step(average) L4 cache hit overlap ratio'))
      inst.vals['cache_miss_overlap_ratio'] = handle_nan(inst.get_val('Step(average) L4 cache miss overlap ratio'))
      ...
      inst.vals['Wght.R'] = inst.get_val('Size.R') / 1 * 100
      inst.vals['Wght.C'] = inst.get_val('Size.C') / 1 * 100
