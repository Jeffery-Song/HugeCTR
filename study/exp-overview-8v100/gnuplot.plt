...
col_dataset                      = 5
# col_pipeline                     = 6
col_step_time_feat_copy   = 7
col_step_time_train_total = 8
# col_step_time_sample             = 9
# col_step_time_recv               = 10
# col_Time.L                       = 11
# col_Time.R                       = 12
# col_Time.C                       = 13
# col_Wght.L                       = 14
...
plot cmd_filter_dat_by_policy(app, ds, bs, "^Rep")         using (column(col_cache_percent) > cache_percent_lb ? column(col_cache_percent) : 1/0):(column(col_step_time_feat_copy)*1000) w lp ps 0.5 lw 1 lc 3 title "Rep" \
    ...
    # ,cmd_filter_dat_by_policy(app, ds, bs, "MPSPhaseRep")  using (column(col_cache_percent) > cache_percent_lb ? column(col_cache_percent) : 1/0):(column(col_step_time_feat_copy)*1000) w lp ps 0.5 lw 3 lc 3 title "MPSRep" \
    # ,cmd_filter_dat_by_policy(app, ds, bs, "MPSPhaseCliq") using (column(col_cache_percent) > cache_percent_lb ? column(col_cache_percent) : 1/0):(column(col_step_time_feat_copy)*1000) w lp ps 0.5 lw 3 lc 2 title "MPSCliqPart" \
}
