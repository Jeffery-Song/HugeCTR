...
args["iter_num"] = 48000                           # the number of training iteration
        ...
        for i in range(num_slots):
            ...
            while True:
                ...
                if len(rnd_rst >= num_samples):
                    break
                print("retry")
            keys_per_slot = np.floor(rnd_rst[:num_samples]).astype(key_dtype) + vocab_range[0]
...
def main():
    ...
    def custom_shard_func(cat, cont, label):
        # input is batched, so the first dim is batch, second dim is one of 13 dense features
